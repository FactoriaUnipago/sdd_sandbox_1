...
# Note: $LIB_DIR points to .sdd-sync-temp/ which doesn't exist yet (cloned with token).
# Use cached lib as fallback, then inline Python as last resort.
if [ -z "$TOKEN" ] && [ -f ".sdd-credentials.json" ]; then
  CRED_LIB=""
  if [ -f "$LIB_DIR/credential_manager.py" ]; then
    CRED_LIB="$LIB_DIR/credential_manager.py"
  elif [ -f ".sdd-cache/sync/lib/credential_manager.py" ]; then
    CRED_LIB=".sdd-cache/sync/lib/credential_manager.py"
  fi

  if [ -n "$CRED_LIB" ]; then
    TOKEN=$($PYTHON "$CRED_LIB" get SDD_SYNC_TOKEN 2>/dev/null | tr -d '\r' || echo "")
  fi

  # Last resort: inline Python (no dependency on credential_manager.py)
  if [ -z "$TOKEN" ]; then
    TOKEN=$($PYTHON -c "import json; print(json.load(open('.sdd-credentials.json')).get('SDD_SYNC_TOKEN',''))" 2>/dev/null | tr -d '\r' || echo "")
  fi
...
# --- 3d2. Validation scripts ---
SCRIPTS_DIR="sdd-scripts"
mkdir -p "$SCRIPTS_DIR"
for script in validate-config.py validate-specs.py validate-language.py; do
  if [ -f "$TEMP_DIR/scripts/$script" ]; then
    cp -f "$TEMP_DIR/scripts/$script" "$SCRIPTS_DIR/$script"
  fi
done
# Copy config template for validation
if [ -f "$TEMP_DIR/core/env/.sdd-config.template.json" ]; then
  cp -f "$TEMP_DIR/core/env/.sdd-config.template.json" ".sdd-config.template.json"
fi
SCRIPTS_COUNT=$(ls -1 "$SCRIPTS_DIR/validate-"* 2>/dev/null | wc -l)
echo "   ✅ Validation scripts: $SCRIPTS_COUNT scripts"
...
if [ ! -f "$PRODUCT_DIR/product.md" ]; then
  mkdir -p "$PRODUCT_DIR"
  cat > "$PRODUCT_DIR/product.md" << 'PRODUCT_EOF'
# Product Overview

> ⚠️ This is a placeholder. Generate the full product.md by running one of:
> - `project-scanner` (for existing projects — scans codebase and fills automatically)
> - `project-initializer` (for new projects — interactive setup)
>
> Do NOT fill this manually — let the agent generate it from your actual project files.
PRODUCT_EOF
  echo "   📝 product.md placeholder creado — usa project-scanner o project-initializer para llenarlo"
fi
...
declare -A CRED_PROMPTS=(
  ["AZURE_DEVOPS_ORG_NAME"]="Organización en Azure DevOps (ej: unipagosa — de dev.azure.com/NOMBRE)"
...
ROLE_ASK=""
case "$ROLE" in
  analyst)   ROLE_ASK="AZURE_DEVOPS_ORG_NAME AZURE_DEVOPS_PAT AZURE_DEVOPS_PROJECT" ;;
  qa)        ROLE_ASK="AZURE_DEVOPS_ORG_NAME AZURE_DEVOPS_PAT AZURE_DEVOPS_PROJECT POSTMAN_API_KEY" ;;
  developer) ROLE_ASK="AZURE_DEVOPS_ORG_NAME AZURE_DEVOPS_PAT AZURE_DEVOPS_PROJECT" ;;
  all)       ROLE_ASK="AZURE_DEVOPS_ORG_NAME AZURE_DEVOPS_PAT AZURE_DEVOPS_PROJECT POSTMAN_API_KEY" ;;
esac
...
for cred_key in $ROLE_ASK; do
  ...
  if [ -z "$CRED_VAL" ]; then
    ...
    if [ -n "$CRED_INPUT" ]; then
      ...
      if [[ "$cred_key" == "AZURE_DEVOPS_ORG_NAME" ]] || [[ "$cred_key" == "AZURE_DEVOPS_PROJECT" ]] || [[ "$cred_key" == "DB_HOST" ]] || [[ "$cred_key" == "DB_PORT" ]] || [[ "$cred_key" == "DB_NAME" ]]; then
...
if [ "$HAS_GIT_IDENTITY" = true ]; then
  ...
  COMMIT_DIRS="$TARGET_DIR .githooks sdd-sync.sh .sdd-config.json .sdd-config.template.json .gitignore .sdd-standards-version .sdd-credentials.template.json specs sdd-scripts product.md design-system-theme.md docs/architecture"
  if [ "$MULTI_IDE" = true ]; then
    for extra_ide in $ALL_IDES; do
      [ "$extra_ide" = "$IDE" ] && continue
      EXTRA_DIR="${IDE_DIRS[$extra_ide]}"
