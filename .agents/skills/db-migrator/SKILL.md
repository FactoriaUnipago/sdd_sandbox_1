...
## ⚠️ Step 0 — MCP Pre-flight (BEFORE any planning database migration)

**BLOCK**: Do NOT start planning database migration until you have completed this step.

1. ☐ Verify `context7` is reachable (test query for ORM/migration tool syntax). If unavailable → note "[context7 unavailable]"
2. ☐ Verify `postgresql` is reachable (test connection to target DB). If unavailable → note "[postgresql unavailable — BLOCK: cannot inspect current schema]"
...
| 2. Inspect Current DB | Connect to target DB (DEV/CERT only). → Use `postgresql` to obtain current schema. → If `context7` available: verify ORM/migration tool syntax |
...
| 10. Verify | Run verification checks (see Verification section). → Use `postgresql` to run post-migration validation queries |
...
| 12. Save to memory | → Use `server-memory` to persist migration history and schema state |
...
## Naming Conventions
- **Tables**: singular, lowercase, snake_case → `empleado`, `solicitud_afiliacion`, NOT `empleados`, `SolicitudAfiliacion`
- **Columns**: lowercase, snake_case → `fecha_nacimiento`, `tipo_documento`, NOT `FechaNacimiento`
- **Primary keys**: `id` (simple) or `{table}_id` (composite)
- **Foreign keys**: `{referenced_table}_id` → `empleador_id`, `solicitud_id`
- **Indexes**: `idx_{table}_{column}` → `idx_empleado_tipo_documento`
- **Constraints**: `uq_{table}_{column}`, `chk_{table}_{rule}`, `fk_{table}_{ref}`
