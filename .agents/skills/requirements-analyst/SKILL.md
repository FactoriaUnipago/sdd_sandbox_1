...
description: Turns the agent into an analysis assistant that generates requirements using EARS notation.. Triggers: requirement, requisito, EARS, acceptance criteria, feature, analysis
---
...
0. ☐ **Determine position** → BEFORE anything else, run these checks IN ORDER. **First match wins — STOP checking.**

   | # | Check | How | 📍 Position |
   |---|---|---|---|
   | 1 | `search_nodes` by WI ID → `task_completion_*` has `status: completed` | Memory | `📍 Crear design.md (⚠️ requiere developer)` |
   | 2 | `requirements.md` exists AND has `[x] Aprobación Analista` | File | `📍 Crear design.md (⚠️ requiere developer)` |
   | 3 | `requirements.md` exists AND has `[ ] Aprobación Analista` | File | `📍 Aprobar requirements.md` |
   | 4 | `requirements.md` does NOT exist | File | `📍 Crear requirements.md` |

   ⚠️ RULES:
   - `[ ] Aprobación Developer` is NOT your concern — ignore it
...
0b. ☐ **Read `docs_language`** → from `.sdd-config.json`. ALL generated content (files + ADO fields) MUST be in this language. Default: `"es"`. EARS keywords (SHALL/WHEN/WHILE/WHERE/IF) stay English.
1. ☐ **WI exists?** → If not, → use `azure-devops` MCP: `create_work_item` with `System.AssignedTo` ← current user email (`git config user.email`). Inform: WI created + assigned.
   - If type = Hotfix → also ask "¿Quién lo arregla?" immediately → save `developer_email` (fast-track)
...
3. ☐ **Scope check (on feature branch)** → If already on a `feature/*` branch with existing `requirements.md`:
   ...
   - Present recommendation with reason: "Recomiendo [X] porque [reason]. ¿De acuerdo?"
...
4. ☐ **Input mode?** → Read `project_type` from `.sdd-config.json` and suggest accordingly:
   - `new` → suggest: verbal idea, doc, or entrevista
   - `existing` (first time SDD) → ask: "¿Documentamos features existentes del código, o agregamos features nuevos?"
   - `existing` (has specs/) → normal: verbal, doc, entrevista, ADO backlog
   - `migration` → suggest: "Escaneo el código para documentar features que necesitan paridad" (mode #7)

4b. ☐ **Interview mode rules** (when user picks "entrevista"):
   - Ask 3-5 focused questions about the feature scope
   - ⚠️ **RESPOND to each answer individually**. If user answers with a counter-question ("¿cuál recomiendas?"), answer it FIRST with your recommendation + reasoning, THEN confirm: "¿vamos con eso?"
   - Do NOT batch-ignore answers. Process each response before moving on.
   - After all questions answered → **summarize what you understood** in 3-5 bullets and ask: "¿Estamos alineados o ajustamos algo?"
...
5. ☐ **Propose REQ list** → Draft initial list of REQs (ID + title + one-line EARS rule). Show as summary table. → Use `sequential-thinking` for complex requirement decomposition (multiple actors, conditional flows, state machines).
6. ☐ **Ask review mode** → "¿Los detallamos uno por uno o todos juntos?"
   - **Incremental** (recommended for ≥3 REQs) → detail each REQ one at a time: EARS rule + acceptance criteria + ask "¿OK o ajustamos?" before next
...
6b. ☐ **Present supporting sections** → BEFORE final generation, proactively present:
   - **Supuestos/Assumptions**: technical assumptions, stubs, constraints (from template §Supuestos)
   - **Limitaciones/Out of Scope**: what is NOT included in this feature
...
7. ☐ **Generate requirements** → Copy `core/templates/requirements-template.md` as scaffold and fill ALL sections:
   - `# Requirements: [Feature Name]` — title header (MANDATORY)
   - `> 📋 Generated by` + `> ✅ Approved by` — header block
   - `## Trazabilidad` — ONE section only (not duplicated). Use Spanish name. Include parent WI link from the start (the parent Feature/Epic is already known from the branch name):
     ```markdown
     ## Trazabilidad
     - **Work Item**: _(pendiente — se agrega al crear el Requirement WI en step 9b)_
     - **Parent**: [AB#{parent_id}](https://dev.azure.com/{org}/{project}/_workitems/edit/{parent_id})
     - **Branch**: `feature/{prefix}{ID}-{name}`
     ```
     ⚠️ **Work Item** = el Requirement HIJO (el que este archivo documenta). NUNCA poner aquí la Feature.
     ⚠️ **Parent** = la Feature o Epic PADRE (de donde nació este requirement). NUNCA poner aquí el Requirement.
     ⚠️ The parent ID is known from step 1 (branch name / WI query). Do NOT wait until step 9b to add it.
   - `## Approval` — formal approval checkboxes
   - `## Overview` — brief feature description (MANDATORY)
   - `### REQ-NNN:` — EACH REQ with this exact format:
     ```
     ...
     **Priority:** [Must|Should|Could]
     ```
   - `## Diagrams` — separate section (NOT mixed with REQs)
   - `## Dependencies` — table format from template
   - `## Supuestos` — table format from template
   - `## Limitaciones` — out of scope items
   - `## Approval` — formal approval section
   ⚠️ Do NOT improvise format. Every generated requirements.md MUST follow the same structure.
8. ☐ **Wait for approval** → User must say approved/OK
9. ☐ **Mark `- [x]`** in `## Approval` section of the file → Replace `_____` with approver name from `git config user.name` + current date. Example: `- [x] Analista: Ana García Fecha: 2026-07-05`
9b. ☐ **Create child Requirement WI** → ⚠️ Duplicate check: Before creating, query ADO by title prefix with WIQL CONTAINS (case-insensitive). If found → skip creation.
     ⚠️ GATE: Do NOT skip. Do NOT update Feature directly.
    - Query tipo del WI actual con `wit_get_work_item`
    - Si tipo = Feature o Epic:
      ...
      d. Title: "[producto] [módulo] REQ: [nombre del requirement]" (⚠️ all brackets lowercase)
      e. Description in **Markdown** format (set `format: "Markdown"` in `wit_create_work_item`):
         ```markdown
         ## Requerimientos ({module})
         
         - **REQ-001:** WHERE un usuario...
         - **REQ-002:** WHEN un usuario...
         
         ---
         
         [Ver documento requirements.md en GitHub]({repo_url}/blob/{branch}/specs/{prefix}{ID}-{name}/requirements.md)
      ...
      g. State: use state discovery (§2b.4) → set to first `InProgress` state matching analysis phase. ⚠️ NEVER leave as "Nuevo" — the WI was just analyzed, it must be InProgress.
      ...
      i. Custom fields from discovery §2b — present ALL at once, proactively, as part of the WI creation flow:
         - **Picklists**: PROPOSE the most logical value based on context + show 2-3 alternatives inline. If >5 options, do NOT list all — just propose + "(otras: X, Y, Z)". Example: `Prioridad: **Alta** (otras: Media, Baja)`
         ...
         - Format: present as a compact confirmation block, NOT as a questionnaire:
           ```
           Para crear el Requirement en ADO:
           - Prioridad: **Alta** _(por impacto transversal)_
           - Categoría: **Funcional**
           - Origen: **Proyecto**
           - Fecha: **2026-07-04**
           ¿OK o ajustamos algo?
           ```
         ⚠️ Do NOT wait for user to ask. Do NOT dump raw field names with "(ej. ...)". PROPOSE values based on analysis.
    - Si tipo = Requirement → update existing WI with content above
    - Guardar `requirement_wi_id` para step 9 y para downstream powers
...
10. ☐ **Update parent WI (ANTES de commit)** → Ejecutar en orden:
   ...
   f. ☐ File link en Description ← URL al requirements.md en repo
   
   ⚠️ GATE: Si ADO falla → informar error exacto. NO hacer commit sin haber intentado ADO.
   Si MCP no disponible → informar "ADO no actualizado" + continuar con commit.
11. ☐ **Commit + push** → Ask user, then `git add + commit + push`
12. ☐ **Save to memory** → `create_entities` or `add_observations` in server-memory:
...
13. ☐ **Assignment** → ⚠️ GATE: Do NOT skip this step. Ask BEFORE showing final position.
    ...
    - Else → ask ALL 3 questions (not just developer). Present together:
      a. "¿Quién implementa? (nombre o email)" → `System.AssignedTo` + `Custom.responsableDesarrollo`
      b. "¿Quién hace QA? (nombre o email, o 'después')" → `Custom.responsableSQA`
      c. "¿Contacto de negocio? (nombre o email, o 'después')" → `Custom.ResponsableNegocio`
      ...
      f. Update `requirements.md` → add/update `## Responsables` table:
         ```
         ## Responsables
         | Rol | Nombre/Email |
         |---|---|
         | Analista | {analyst_email} |
         | Desarrollador | {developer_email} |
         | QA | {qa_email} |
         | Negocio | {business_email} |
         ```
14. ☐ **COMPLETION CHECKPOINT** → Before showing 📍, verify ALL of the following are done:
    ...
    - [ ] Responsables asked + saved (step 13) — identity fields in WI + `## Responsables` in file
...
- Present recommendation marked. If ambiguous: "Parece [X] porque [reason]. ¿Cuál prefieres?"
...
|---|---|---|
| Feature, Migration, Release | `core/templates/requirements-template.md` | `specs/{prefix}{ID}-{name}/requirements.md` |
| Fix, Hotfix | `core/templates/bugfix-template.md` | `specs/{prefix}{ID}-{name}/bugfix.md` |
...
7. **Scope proposal** — After approval, show REQ titles (numbered) and ask: "¿Genero todos o prefieres ir subiendo de a poco?"
...
8. **Drill-down review** — After saving, ask: "¿Quieres revisar algún REQ en detalle?" Show only that REQ's full content. User adjusts → update file. Loop until done. THEN commit + assignment.
...
- requirements.md MUST start with a traceability header:
  ...
  ## Traceability
  - **Work Item**: [{prefix}{ID}]({tracker_url})        ← Requirement WI (HIJO)
  - **Parent**: [{parent_type} {parent_prefix}{parent_ID}]({parent_tracker_url})  ← Feature/Epic (PADRE)
  - **Branch**: `feature/{prefix}{ID}-name`
  ```
  ⚠️ **REGLA DE JERARQUÍA**: `Work Item` SIEMPRE apunta al WI que ESTE archivo documenta (Requirement). `Parent` SIEMPRE apunta al WI del nivel superior (Feature/Epic). NUNCA invertirlos.
...
When a requirement references a capability that belongs to another feature (e.g., "usuario autenticado" but Auth is a separate feature):
...
2. **Document** — Add `## Dependencies` section in requirements.md:
   ...
   |---------|------------------|--------|------------|
   | [Auth] AB#XXX | JWT validation | ⬜ No existe | Header `X-User-Id` (stub) |
   | [Payments] AB#YYY | Payment processing | ✅ En producción | — |
   ```
3. **Ask user** — "Este feature menciona [capability]. ¿Ya existe o necesita stub temporal?"
   - **Ya existe** → documentar como dependencia normal
   - **No existe** → documentar stub/workaround (ver abajo)
   - **Se hará después** → documentar como feature futuro + stub temporal

## Supuestos y Stubs Temporales
...
|---|----------|---------------|---------------------|
| A1 | El usuario está autenticado | Header `X-User-Id` en request | Feature Auth (AB#XXX) implementado |
| A2 | Existe servicio de email | Log a consola: `[EMAIL] To: X, Subject: Y` | Feature Notifications implementado |
```

**Rules:**
- **NEVER** leave implicit assumptions — if REQ says "usuario autenticado" and auth doesn't exist, the developer WILL block or invent auth (scope creep)
...
- In NFR section, add: `NFR-STUB: Los stubs temporales deben ser removibles en menos de 1 hora cuando el feature real se implemente`

## Common Technical Assumptions Checklist

Antes de presentar requirements para aprobación, verificar si los REQs implican estos supuestos técnicos. Si aplican → agregar a `## Supuestos`:

| Si los REQs mencionan... | Preguntar / Documentar | Supuesto default |
|---|---|---|
| Fechas, timestamps, vencimientos | ¿Zona horaria? | UTC (ISO 8601) |
| IDs, identificadores en API | ¿Formato de ID? | UUID v4 (no auto-increment) |
| Listas, búsquedas, resultados | ¿Paginación? | Cursor-based, limit default 20, max 100 |
| Errores, validaciones | ¿Formato de error? | JSON `{ error: { code, message, details } }` |
| Usuarios, roles, permisos | ¿Auth existe? | Ver §Cross-Feature Dependencies |
| Archivos, uploads | ¿Tamaño máximo? | 10MB default, tipos permitidos |
| Montos, dinero, precios | ¿Moneda? ¿Precisión decimal? | DOP, 2 decimales |
| Emails, notificaciones | ¿Servicio de email existe? | Ver §Cross-Feature Dependencies |
| Búsqueda de texto | ¿Full-text o exacta? | Exacta (LIKE) default |
| Idioma del contenido | ¿Multi-idioma? | Single (docs_language) |

**Regla**: Si un supuesto aplica y NO está documentado → agregarlo a `## Supuestos` antes de pedir aprobación. El developer NO debe adivinar.
...
1. Ask: email/name/"sin asignar"
2. `core_get_identity_ids` → resolve identity
3. Confirm: "¿Asigno a [displayName] ([email])?"
4. Confirmed → `wit_update_work_item`: `System.AssignedTo` = email
5. No match → "⚠️ No encontré '[value]'. ADO busca mejor por email completo. ¿Tienes el correo?"
6. "sin asignar" → leave unassigned
