---
name: Code Reviewer
description: Automated code review following project standards. Identifies changed files, evaluates against checklist, generates report with findings classified by severity with references to specific lines.. Triggers: review, code review, pr, pull request, security, quality, standards
---

## ⚠️ Step 0 — MCP Pre-flight (BEFORE any reviewing code)

**BLOCK**: Do NOT start reviewing code until you have completed this step.

1. ☐ Verify `context7` is reachable (test query for library API verification). If unavailable → note "[context7 unavailable]"
2. ☐ Call `codebase-memory` → verify project is indexed (risk mapping, dead code detection). If unavailable → note "[codebase-memory unavailable]"
...
| 0b | ☐ **Read `docs_language`** → from `.sdd-config.json`. Default: `"es"`. ALL output (reports, ADO fields) in this language. |
...
| 8 | ☐ **PR** → Does a PR exist? If not: create_pull_request (source → target, reviewers, WI link). If yes: verify linked WI. → Use `github` to create/read PR, post review comments |
...
### ⚠️ MCP Self-Verification (verify BEFORE presenting review report to user)

- ☐ Called `codebase-memory` for risk mapping, dead code, near-clone detection, and impact analysis (step 3)
- ☐ Ran `eslint` on all changed JS/TS files and included violations in report (step 4)
- ☐ Called `context7` to verify correct API usage of external libraries (step 4)
- ☐ Read stored conventions and prior corrections from `server-memory` (step 2)
- ☐ If any MCP unavailable → noted as `[NOT VERIFIED]` in review report
