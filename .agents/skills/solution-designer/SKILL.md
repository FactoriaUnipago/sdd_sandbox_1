...
description: Creates design docs and tasks from approved requirements.. Triggers: design, diseño, architecture, task, data model, api contract, ui prompt, ui design
---

## ⚠️ MANDATORY CHECKLIST — verify EVERY step

0. ☐ **Read `docs_language`** → from `.sdd-config.json`. ALL generated content (files + ADO fields) MUST be in this language. Default: `"es"`. Technical terms (API, DB, JWT, etc.) stay English.
1. ☐ **Requirements approved?** → Check `## Approval` has `- [x]`. If `- [ ]` → **BLOCK**: "Requirements necesita aprobación primero."
...
2c. ☐ **Propose key architecture decisions** → BEFORE generating anything, present the main decisions for user input. Use `sequential-thinking` for complex tradeoffs (monolith vs microservices, SQL vs NoSQL). Use `context7` to verify library/framework docs, latest stable versions, and compatibility before proposing stack:
   - **Stack/Language**: "Para este módulo propongo [X] porque [razón]. ¿OK o prefieres otra cosa?"
   - **Database**: "Tabla [X] con columnas [Y]. ¿Está bien o necesitas algo diferente?"
   - **API pattern**: "REST con [N] endpoints: [lista]. ¿Ajustamos?"
   - **Auth/Security**: "Middleware [X] para [razón]"
   - **Deployment**: Proponer basado en stack (ver `deployment-patterns.md`):
      - Leer `deployment` de `.sdd-config.json` — si ya existe, proponer mantener o cambiar
      - Si no existe → mapear stack → deployment recomendado
      - Si `project_type: "existing"` → detectar deployment actual, proponer mantener o migrar
      - **Mobile targets**:
   ...
   - **Infrastructure**: Servicios AWS basados en `deployment.infra` y reqs. → Use `aws-docs` to verify service capabilities/limits. Use `aws-pricing` to estimate costs before committing.
   
   Present as a compact proposal with **justificación por línea** + **descartados con razón**:
   ```
   Propuesta de arquitectura para [Module]:
   
   📦 Stack:
   - **Node.js + Express** _(consistente con [core] que ya usa Express)_
   - **PostgreSQL** _(misma instancia RDS de [core] — evita latencia cross-service)_
   ...
   _(React + Express juntos → Vercel nativo, sin config extra.
   Separar en monorepo-split agregaría complejidad innecesaria.
   Mobile: Capacitor → TestFlight/Firebase App Distribution. Flutter → Fastlane.)_
   
   ☁️ Infra AWS:
   - **RDS** → tabla `notifications` en misma instancia _(evita latencia)_
   - **SES** → envío de emails _(más barato que SendGrid < 62K/mes)_
   - **EventBridge** → cron cada 1h _(serverless, no necesita instancia 24/7)_
   
   ❌ Descartados:
   - SendGrid → más caro, proveedor externo innecesario
   - monorepo-split → overhead de 2 pipelines para proyecto pequeño
   
   ¿OK o ajustamos algo?
   ```

   ⚠️ If user asks counter-questions ("¿cuál recomiendas?", "¿por qué no X?") → answer with reasoning FIRST, then re-confirm.
...
3. ☐ **Generate design.md** → ONLY AFTER architecture confirmed. Read `core/templates/design-template.md` FIRST, use as scaffold. Read `project_type` from `.sdd-config.json`:
   ...
   ⚠️ Do NOT improvise structure. Read the template, copy its headers, fill content.
...
6. ☐ **Mark `- [x]`** in `## Approval` section of design.md → Replace `_____` with approver name from `git config user.name` + current date. Example: `- [x] Developer: Ana García Fecha: 2026-07-05`
...
8. ☐ **Update ADO (design artifacts)** → Tag `design-approved` on Requirement WI + Story Points ← sum of estimates
   ...
   - Create **Modelo de Datos** WIs (skip if frontend-only / §3.2 empty):
     - 1 per DB table from §3.2. Title: `[producto] [módulo] MD: {table_name}`
     ...
     ⚠️ Follow exact template from azure-devops-workflow.md §Description Format — Modelo de Datos.
   ...
   - Create **Servicio** WIs (skip if frontend-only / §3.3 empty):
     - 1 per endpoint group from §3.3. Title: `[producto] [módulo] SVC: {METHOD} {route}`
     - Description: auth, request/response schemas (use inline-styled HTML tables for schemas), errors, curl example
     ⚠️ Follow exact template from azure-devops-workflow.md §Description Format — Servicio.
     - `format: "HTML"` if description contains tables, otherwise `format: "Markdown"`
     - Parent link: AFTER creation, call `wit_work_items_link` (see azure-devops-workflow.md §5). Do NOT use `System.Parent` field.
   - Create **Estructuras de Datos** WIs (skip if no schemas in §3.1):
     - 1 per major data schema. Title: `[producto] [módulo] ED: {schema_name}`
     - Description: HTML table with inline styles — fields, types, relationships
     ⚠️ Follow exact template from azure-devops-workflow.md §Description Format — Estructuras de Datos.
     - `format: "HTML"` in `wit_create_work_item`
     - Parent link: AFTER creation, call `wit_work_items_link` (see azure-devops-workflow.md §5). Do NOT use `System.Parent` field.
   ...
   - **State**: use state discovery (§2b.4) → set to first `InProgress` state. ⚠️ NEVER leave as "Nuevo".
...
12. ☐ **Generate tasks.md** → Read `core/templates/tasks-template.md` FIRST, use as scaffold. Implementable task list from design.
   ...
   - ⚠️ **Environment detection for Wave 0** — PROACTIVE, not reactive. Do NOT propose options one by one waiting for rejection.
     ...
     **Step B — Ask ONCE** with only the options that are actually available:
     ```
     🔧 Detecté: [Docker ✅/❌] · [PG local ✅/❌] · [AWS CLI ✅/❌]
     Tu proyecto usa: [deployment.db from config]

     Para desarrollo local, estas son tus opciones:
     ...
     **Step C — If NOTHING is available**, present installation options ranked by effort:
     ```
     ⚠️ No tienes Docker, DB local, ni CLI de cloud configurado.
     Opciones para arrancar:
     1. 🌐 DB cloud gratuita (Neon/Supabase) — 2 min, solo signup + copiar connection string
     2. 🐳 Instalar Docker Desktop — ~15 min, recomendado para dev serio
     3. 🐘 Instalar PostgreSQL nativo — ~10 min, proceso en background permanente
     4. ⚠️ SQLite temporal — 0 min setup, PERO con limitaciones (ver abajo)
     ¿Cuál prefieres?
     ```
     **SQLite como último recurso** — SOLO si el usuario lo elige explícitamente. SIEMPRE advertir:
     - ⚠️ NO es "transparente". Diferencias: sin ENUM, sin CHECK constraints complejos, sin arrays, tipos de fecha distintos, sin JSONB.
     - ⚠️ Las migraciones generadas para SQLite NO servirán en producción PostgreSQL. Habrá que regenerarlas.
     - ⚠️ Marcar en tasks.md: "DB: SQLite (temporal) — migrar a PostgreSQL antes de CERT".
   - ⚠️ **IaC + Pipeline question** (only if greenfield): Ask the user:
     - "¿Configuro infraestructura como código (CDK/Terraform) desde ahora, o lo dejamos para después?"
     - "¿Configuro el pipeline de CI/CD desde ahora?"
...
17. ☐ **Save to memory** → Persist design decisions to server-memory
...
0b. **Use templates** — If `specs/_templates/design.md` exists, use as base structure. If not found, use `core/templates/design-template.md`.
...
0c. **Traceability header** — design.md MUST start with:
    ...
    ## Trazabilidad
    - **Work Item**: [{prefix}{ID}]({tracker_url})        ← Requirement WI (MISMO que requirements.md)
    - **Parent**: [{parent_prefix}{parent_ID}]({parent_tracker_url})  ← Feature/Epic (PADRE)
    - **Rama**: [tipo]/{prefix}[ID]-[nombre]
    - **Requirements**: [requirements.md](./requirements.md)
    - **Test Plan**: [test-plan.md](./test-plan.md)
    ```
    ⚠️ **Work Item** = el MISMO Requirement WI que requirements.md. design.md documenta EL MISMO requirement, no crea un WI nuevo.
    ⚠️ **Parent** = Feature/Epic. NUNCA poner aquí el Requirement.
...
|---|---|
| API contracts | `references/api-contract-template.md` |
...
> **¿Los detallamos uno por uno o todos juntos?**
...
If `docs/architecture/` missing → create from `_templates/*-template.md`. Skip if no changes.
