...
**⚠️ ANTI-FLIP-FLOP**: When you give a recommendation or technical opinion, HOLD your position. If the user questions or pushes back, explain your reasoning — do NOT reverse just because the user disagreed. Only change if the user provides NEW information that genuinely changes the analysis. Flip-flopping destroys trust.

**⚠️ sdd-config.json**: NEVER edit `.sdd-config.json` manually. Use `sdd-sync.sh`:
- **Role change** → `bash sdd-sync.sh --role [role] --force` — ALWAYS use `--force` because the standard version is typically already up to date. Do NOT run without `--force` and wait for "versions match" error.
- **Standard update** → `bash sdd-sync.sh --force` — use `--force` to pull latest even if versions appear equal (patches may exist).
- The script handles config update + commit atomically.

**⚠️ RETROFIT**: When touching ANY spec file (create OR update), verify:
1. Traceability header exists → if missing, ADD IT
2. Footer exists → if missing, ADD IT
3. ADO WI has repo link in Description → if missing, `update_work_item` to add it
4. ADO WI fields are current (Acceptance Criteria (fallback: Description), tags) → if stale, update
No spec file goes without traceability — in the repo AND in ADO.

**⚠️ CONSULT → CONFIRM → GENERATE**: Before creating ANY spec file, the power MUST:
1. **Read inputs** → Previous artifacts (requirements.md → design.md → tasks.md → test-plan.md)
2. **Propose key decisions** → Present a compact proposal block with the main decisions and reasoning. NOT a questionnaire — propose values proactively.
3. **Handle counter-questions** → If user asks "¿cuál recomiendas?" or "¿por qué no X?" → answer with reasoning FIRST, then re-confirm.
4. **Get explicit confirmation** → User must approve the proposal before file generation.
5. **ONLY THEN generate** → Create the spec file based on confirmed decisions.
...
**ADO updates = FIELDS, not just comments.** When content changes (REQs, design, repo links), update the WI **fields** (Description, Acceptance Criteria (fallback: Description)). Comments are ONLY for audit trail ("what changed and why"). A comment alone without updating the fields is incomplete.

**⚠️ CONTEXT ISOLATION**: NEVER read, write, or reference files outside the current project's root directory. If the IDE sends events from another project/workspace, IGNORE them. Your context is the project where `.sdd-config.json` lives — nothing else.
...
- **Analyst**: requirements-analyst, backlog-manager, sprint-planner, release-notes-writer, test-data-generator
...
6. ☐ **Chat output** → `✅ Guardado → 📄 [file](file:///path)` + approval + `📍 [next]`

> **Do NOT skip any step.** All 6 are mandatory every time a spec is saved.
